class AdminController < InheritedResources::Base
  ...
  def add_instructor
    ...
    InstructorsCourses.where(course_id: course.id, user_id: @instructor.id).first.update_attribute(:uuid, uuid)
    UsersMailer.instructorNotificationEmail(@instructor, course, uuid).deliver
      ...
      if charges.first
        @charges.push(
          { month: seed.strftime('%B %Y'),
            ...
            total_net_month: total_amount_month - total_fee_month
          })
      end
    }
  end
